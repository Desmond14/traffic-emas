...
calculates_dist_to_car_on_the_same_lane_test() ->
  ...
  [Car | _] = input:load_car_definitions("basic.cars"),
  calculate_dist_to_car_ahead_test(UpdatedIntersection, Car, 3).

calculates_dist_to_car_on_same_position_test() ->
  Intersection = intersection:add_car_on(5, #{node_id=>1, position_on_node=>1}, input:load_intersection_definition("basic.intersection")),
  [Car | _] = input:load_car_definitions("basic.cars"),
  calculate_dist_to_car_ahead_test(Intersection, Car, 0).

calculates_dist_to_car_on_semaphore_test() ->
  Intersection = intersection:add_car_on(5, #{node_id=>8, position_on_node=>1}, input:load_intersection_definition("basic.intersection")),
  [Car | _] = input:load_car_definitions("basic.cars"),
...
calculates_dist_to_car_on_next_lane_test() ->
  Intersection = intersection:add_car_on(5, #{node_id=>11, position_on_node=>1}, input:load_intersection_definition("basic.intersection")),
  [Car | _] = input:load_car_definitions("basic.cars"),
