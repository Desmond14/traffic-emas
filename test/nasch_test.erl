...
should_accelerate_by_1_test() ->
  ...
  {UpdatedCar, _} = nasch:follow_nasch(Car, Intersection),
...
should_add_on_new_position_test() ->
  Intersection = input:load_intersection_definition(),
  Car = hd(input:load_car_definitions()),
  {_, UpdatedIntersection} = nasch:follow_nasch(Car, Intersection),
...
should_decelerate_by_1_test() ->
  ...
  {UpdatedCar, _} = nasch:follow_nasch(Car, UpdatedIntersection),
...
should_return_outside_intersection_test() ->
  ...
  {UpdatedCar, _} = nasch:follow_nasch(CarAtVeryEnd, UpdatedIntersection),
