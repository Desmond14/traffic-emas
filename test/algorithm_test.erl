...
should_accelerate_by_1_test() ->
  ...
  {UpdatedCar, _} = algorithm:move(Car, Intersection, #{4=>1}),
...
should_add_on_new_position_test() ->
  Intersection = input:load_intersection_definition("basic.intersection"),
  Car = hd(input:load_car_definitions("basic.cars")),
  {_, UpdatedIntersection} = algorithm:move(Car, Intersection, #{4=>1}),
...
should_not_accelerate_more_than_max_velocity_test() ->
  ...
  Car = car:set_config(CarConfig, hd(input:load_car_definitions("basic.cars"))),
  {UpdatedCar, _} = algorithm:move(Car, Intersection, #{4=>1}),
...
should_return_outside_intersection_test() ->
  ...
  UpdatedIntersection = intersection:add_car_on(5, #{node_id=>11, position_on_node=>5}, input:load_intersection_definition("basic.intersection")),
  {UpdatedCar, _} = algorithm:move(CarAtVeryEnd, UpdatedIntersection, #{4=>1}),
