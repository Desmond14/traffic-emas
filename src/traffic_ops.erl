...
generate_solution(Intersection, ProblemSize) ->
  ...
  generate_solution(InitialLightValue, InitialLightDuration, SemaphoreIds, ProblemSize, []).

-spec generate_solution(trit(), non_neg_integer(), [node_id()], non_neg_integer(), lights()) -> lights().
generate_solution(_InitialLightValue, _InitialLightDuration, _SemaphoreIds, 0, Lights) ->
  Lights;

generate_solution(InitialLightValue, InitialLightDuration, SemaphoreIds, ProblemSize, Lights) ->
  LightCycle = create_lights_cycle(InitialLightValue, InitialLightDuration, SemaphoreIds, ProblemSize),
  generate_solution(InitialLightValue, InitialLightDuration, SemaphoreIds, ProblemSize-length(LightCycle), Lights ++ LightCycle).

-spec create_lights_cycle(trit(), non_neg_integer(), [node_id()], non_neg_integer()) -> lights().
create_lights_cycle(InitialLightValue, SingleLightDuration, SemaphoreIds, TotalLightsDuration) ->
  lists:sublist(
    create_lights(InitialLightValue, SingleLightDuration, SemaphoreIds)
    ++ create_lights(?BOTH_RED, ?SAME_LIGHT_AT_LEAST_FOR, SemaphoreIds)
      ++ create_lights(opposite_light(InitialLightValue), SingleLightDuration, SemaphoreIds)
      ++ create_lights(?BOTH_RED, ?SAME_LIGHT_AT_LEAST_FOR, SemaphoreIds),
    TotalLightsDuration).
